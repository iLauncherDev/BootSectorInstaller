                ...
                void *Dest, void *Src)
{
    ...
    uint32_t ExtGroupDescSize = (DestVBR->FeatureIncompat & EXT4_FEATURE_INCOMPAT_64BIT ? 64 : 32);
    ...
    uint32_t ExtGroupDescPerBlock = ExtBlockSizeInBytes / ExtGroupDescSize;
    uint32_t ExtFirstDataBlock = DestVBR->FirstDataBlock;
    uint32_t ExtInodesPerGroup = DestVBR->InodesPerGroup;
    uint32_t ExtInodesPerBlock = ExtBlockSizeInBytes / ExtInodeSize;
    ...
    printf(DEBUG_STRING "%s = 0x%x;\n", "DestVBR->ExtGroupDescPerBlock", DestVBR->ExtGroupDescPerBlock);
    ...
    printf(DEBUG_STRING "%s = 0x%x;\n", "DestVBR->ExtInodesPerBlock", DestVBR->ExtInodesPerBlock);
